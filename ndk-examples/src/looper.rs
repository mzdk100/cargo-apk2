...
fn android_main(app: AndroidApp) {
    ...
    // 检索 android-activity 在当前线程上为我们创建的 Looper。android-activity 使用它来阻止事件并通过单一机制轮询文件描述符。
    let looper =
    ...
    // 创建一个 Unix 管道，将自定义事件发送到 Looper。ndk-glue 使用类似的机制，通过 NDK_GLUE_LOOPER_EVENT_PIPE_IDENT 将 ANativeActivityCallbacks 异步传递给 Looper。
